---
title: "数据倾斜（Data Skew）解决方案：八种实战解法图文详解"
...
# 数据倾斜（Data Skew）解决方案：八种实战解法图文详解
...
本文用图说话，讲清楚倾斜是怎么发生的，并给出从基础到进阶的**八个实战思路**：**思路一 双层 Group By**、**思路二 热点 Key 单独计算**、**思路三 哈希分组（加盐）Join**、**思路四 转 Broadcast Join**、**思路五 空值归一化**、**思路六 数据预处理**、**思路七 并行度调优**与**思路八 AQE**。
...
| 空值/默认值倾斜 | **过滤 + 归一化**（思路五） | 成本最低，先排查 |
| 不想改 SQL | **AQE 开关**（思路八） | 白嫖优化，先开起来 |
| 长期稳定任务 | **分桶/预聚合**（思路六） | 治本于上游 |
